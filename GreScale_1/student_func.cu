                       ...
                       int numRows, int numCols)
{
  ...
  //calculate a 1D offset
	...
	int idx = blockDim.x * rowIdx + colIdx;
	if (rowIdx < numRows && colIdx < numCols) {
		uchar4 rgba = rgbaImage[idx];
		float intensity = 0.299f*rgba.x + 0.587f*rgba.y + 0.114f*rgba.z;
		greyImage[idx] = intensity;
	}*/

	int idx = (blockIdx.x * blockDim.x + threadIdx.x) +
		(blockIdx.y * blockDim.y + threadIdx.y) * numCols; // Important!
	uchar4 rgba = rgbaImage[idx];
                            ...
                            unsigned char* const d_greyImage, size_t numRows, size_t numCols)
{
  ...
  //currently only one block with one thread is being launched
	...
	//TODO: row*col WRONG! thread num is at most 1024
